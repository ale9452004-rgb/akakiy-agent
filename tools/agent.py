...
class Agent:
    ...
    def process(self, user_input):
        ...
        # 1.4. Создание презентаций (Presentation Sub-Agent Fast-Path)
        if route_type == "presentation":
            prompt = route.get("prompt", "")
            subagent_res = self.run_subagent("presentation", task=prompt, metadata=route)
            if subagent_res.success:
                answer = subagent_res.message
            else:
                err_detail = subagent_res.error or subagent_res.message or "Неизвестная ошибка"
                answer = f"Ошибка создания презентации: {err_detail}"
        ...
        # 1.7. Явный запуск субагента (Sub-Agent Fast-Path)

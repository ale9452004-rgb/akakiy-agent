...
def test_voice_error_reporting():
    ...
    assert "Ошибка" in speech_fail and "не найдена" in speech_fail, f"Got: {speech_fail}"
    assert "успешно" not in speech_fail.lower(), f"Falsely reported success: {speech_fail}"
    ...
    assert "Ошибка" in speech_inner and "не существует" in speech_inner, f"Got: {speech_inner}"
    ...
    assert "Купить молоко" in speech_ok and "Ошибка" not in speech_ok, f"Got: {speech_ok}"
    print(f"  [OK] Success tool speech: '{speech_ok}'")


def test_batch_and_relative_delete(tmp_path: Path):
    print("\n--- 3. Проверка Batch / Relative Deletion ---")
    test_db = tmp_path / "test_household.json"
    mgr = HouseholdManager(storage_path=test_db)

    # Создаём 5 тестовых заметок
    for i in range(1, 6):
        mgr.create_note(f"Заметка {i}", f"Контент {i}")
    assert len(mgr.notes) == 5

    # 1. Относительное удаление: 'последние три'
    res_rel = mgr.delete_note("последние три")
    assert res_rel["success"] is True, f"Failed: {res_rel}"
    assert res_rel["count"] == 3, f"Expected 3 deleted, got {res_rel['count']}"
    assert len(mgr.notes) == 2, f"Expected 2 remaining notes, got {len(mgr.notes)}"
    print(f"  [OK] Delete 'последние три': {res_rel['message']}")

    # 2. Удаление по списку номеров: '#1, #2'
    res_batch = mgr.delete_note("#1, #2")
    assert res_batch["success"] is True, f"Failed: {res_batch}"
    assert res_batch["count"] == 2
    assert len(mgr.notes) == 0, f"Expected 0 remaining notes, got {len(mgr.notes)}"
    ...
    assert res_empty["success"] is False, f"Expected failure on empty collection, got {res_empty}"
    ...
    assert res_task_rel["success"] is True
    assert len(mgr.tasks) == 2
    print(f"  [OK] Delete task 'последнюю': {res_task_rel['message']}")


def test_deterministic_routing_and_latency():
    ...
    for cmd, expected_tool in commands:
        ...
        assert tool == expected_tool, f"Query '{cmd}' routed to '{tool}', expected '{expected_tool}'"
        assert elapsed_ms < 10.0, f"Latency too high: {elapsed_ms:.2f} ms for '{cmd}'"
...
if __name__ == "__main__":
    ...
    try:
        test_wake_word_handling()
        test_voice_error_reporting()
        test_batch_and_relative_delete(tmp)
        test_deterministic_routing_and_latency()
        print("\n==========================================")
        print(" ВСЕ ТЕСТЫ РЕГРЕССИИ 023 УСПЕШНО ПРОЙДЕНЫ!")
        print("==========================================")
    finally:

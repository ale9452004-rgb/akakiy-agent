...
* **Текущая версия**: Акакий 2.0 (Desktop Hub + Voice UX + Household Assistant + Memory + Core Dev Tools + Notifications + Fast CLI REPL + Global Push-to-Talk).
* **Состояние кодовой базы**: Стабильное, все тесты пройдены (`80` файлов валидированы, 206 тестов в `tests/` успешны, 0 синтаксических ошибок).
* **Последний этап**: Глобальный Push-to-Talk для голоса (Ctrl+Shift+Space, Win32 RegisterHotKey).
...
* [x] **Глобальный хоткей Push-to-Talk для голоса**:
  ...
  * Создан набор тестов `tests/test_voice_hotkey.py` (9 тестов, 100% pass, всего 206 тестов в `tests/`).

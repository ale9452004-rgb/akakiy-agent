...
* **Текущая версия**: Акакий 2.0 (Desktop Hub + Voice UX + Household Assistant + Memory + Core Dev Tools + Notifications + Fast CLI REPL + Global Push-to-Talk + Backup & Restore).
* **Состояние кодовой базы**: Стабильное, все тесты пройдены (`82` файла валидированы, 227 тестов в `tests/` успешны, 0 синтаксических ошибок).
* **Последний этап**: Экспорт и импорт пользовательских данных (tools/backup.py, GUI Settings, CLI :export / :import).
...
* [x] **Экспорт и резервное копирование пользовательских данных**:
  ...
  * Создан набор тестов `tests/test_backup.py` (21 тест, 100% pass, всего 227 тестов в `tests/`).

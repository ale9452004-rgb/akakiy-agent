...
* [x] **ResearchAgent v1: специализированный Sub-Agent сбора и анализа информации (Этап 9)**:
  * Разработан специализированный субагент `ResearchAgent` (`tools/agents/research.py`) для структурированного сбора и анализа информации без внешних зависимостей.
  * Реализован метод `parse_task(task, metadata)`: очистка от префиксов, извлечение темы, списка исследовательских вопросов (из метаданных, текста с маркерами или 3 дефолтных ключевых аспектов) и подсказок по источникам.
  * Реализован метод `gather_information(...)`: анализ переданных файлов (`context.files`), сканирование документации проекта и кодовой базы по ключевым словам, извлечение релевантных сниппетов и формирование структурированных выводов.
  * Генерация подробного аналитического отчёта в формате GitHub Flavored Markdown (`generate_report_markdown`) и сохранение в `data/generated/research/report_<timestamp>_<uuid>.md`.
  * В `Artifact` добавлены свойство `is_research` и фабричный метод `from_research`, в `AgentResult` добавлено свойство `research_reports`.
  * `ResearchAgent` зарегистрирован по умолчанию в `AgentRegistry` и экспортирован из `tools/agents`.
  * В `CommandRouter` добавлен метод `match_research(user_input)` для естественных команд и явных вызовов `субагент research: ...`, обновлен `NON_IMAGE_TARGET_PATTERN`.
  * В `Agent.process()` подключена обработка маршрута `route_type == "research"` с запуском субагента, возвратом структурированного ответа и фиксацией в контексте диалога.
  * В `main.py` добавлен вывод отчётов исследования в CLI REPL.
  * Создан модульный тестовый набор `tests/test_research_agent.py` (15 тестов, 100% pass).
  * Всего 459 тестов в `tests/` (457 unit pass + 2 integration skip by default).
* [x] **CodingAgent v1: специализированный Sub-Agent анализа и безопасного изменения кода (Этап 10)**:
  * Разработан специализированный субагент `CodingAgent` (`tools/agents/coding.py`) для анализа кода, выявления дефектов, безопасного точечного применения правок и верификации изменений.
  * Реализован метод `parse_task(task, metadata, context_files)`: извлечение целевых файлов, белого списка разрешённых файлов (`allowed_files`), структуры точечных замен `edits` (`old_str` -> `new_str`), режима (`edit` или `analyze`), флага верификации и опциональной команды тестирования `test_command`.
  * Реализован метод `analyze_files(target_files)`: валидация безопасности путей (защита от path traversal за пределы рабочей директории), проверка существования и доступности файлов, статический анализ синтаксиса Python через `tools.validation.validate_python_file`.
  * Реализован безопасный механизм `apply_edits(edits, allowed_files)`: строгая изоляция модификаций исключительно файлами из `allowed_files` (любые попытки изменить сторонние файлы отклоняются до начала записи), выполнение точечных замен через `tools.files.edit_file`.
  * Реализована целевая верификация `verify_changes(modified_files, test_command)`: автоматическая компиляция синтаксиса изменённых Python-файлов (`validate_python_file`), выполнение опциональных тестовых команд через `tools.terminal.run_command`.
  * В `Artifact` добавлены свойство `is_code` и фабрика `from_code`, в `AgentResult` добавлено свойство `code_artifacts`, обновлён `AgentResult.fail()` с поддержкой `created_files`.
  * `CodingAgent` зарегистрирован по умолчанию в `AgentRegistry` и экспортирован из `tools/agents`.
  * В `CommandRouter` (`tools/router.py`) добавлен метод `match_coding(user_input)` для естественных русскоязычных команд («кодинг: ...», «код: ...», «отредактируй файл ...», «исправь код в файле ...», «проанализируй код ...», «примени патч ...») и вызовов `субагент coding: ...`, расширен `NON_IMAGE_TARGET_PATTERN`.
  * В `Agent.process()` подключена обработка маршрута `route_type == "coding"` с запуском субагента, возвратом структурированного ответа и фиксацией в контексте диалога.
  * В `main.py` добавлен вывод артефактов кода в CLI REPL.
  * Создан модульный тестовый набор `tests/test_coding_agent.py` (18 тестов, 100% pass).
  * Всего 477 тестов в `tests/` (475 unit pass + 2 integration skip by default).
* [x] **FileAgent v1: специализированный Sub-Agent безопасных файловых операций (Этап 11)**:
  * Разработан специализированный субагент `FileAgent` (`tools/agents/file.py`) для поиска, чтения, создания, копирования, перемещения и получения метаданных файлов.
  * Реализован метод `parse_task(task, metadata, context_files)`: гибкое определение действия (`search`, `read`, `create`, `copy`, `move`, `metadata`, `delete`), извлечение путей к файлам, источников/назначений, содержимого и поисковых запросов.
  * Реализованы безопасные методы `read()`, `create()`, `copy()`, `move()`, `get_metadata()`, `search()` со строгой валидацией путей внутри рабочей директории и защитой от path traversal.
  * Реализован категорический запрет на операции удаления файлов: любые запросы `delete`/`remove`/`unlink` немедленно отклоняются политикой безопасности агента с возвратом `AgentResult.fail()`.
  * В `tools/files.py` функции `resolve_safe_path`, `read_file`, `write_file` расширены опциональным параметром `base_path` с сохранением полной обратной совместимости.
  * В `Artifact.from_file` добавлена поддержка параметра `content`, в `AgentResult` добавлено свойство `file_artifacts`.
  * `FileAgent` зарегистрирован по умолчанию в `AgentRegistry` и экспортирован из `tools/agents`.
